...
//hardware decoder
#define HARDWARE_DECODER_CUVID 0
...
int main(int argc, char **argv)
{
	...
	int tmp = 10;
	av_log(NULL, AV_LOG_WARNING,"%s, %d, %s, %s, %s", "input",tmp,"flv","from","filepath");

	Log::Info("%s #%d, %s, %s %s\n", "Input", 0, "flv", "from", "filepath");

#if HARDWARE_DECODER_CUVID
	AVHWAccel *hwaccel = NULL;
	while ((hwaccel = av_hwaccel_next(hwaccel)) != NULL)
	{
		if (strcmp("h264_cuvid", hwaccel->name) == 0)
		{
...
#else
				SDL_UpdateYUVTexture(sdlTexture, &sdlRect,
					pFrame->data[0], pFrameYUV->linesize[0],
					pFrame->data[1], pFrameYUV->linesize[1],
					pFrame->data[2], pFrameYUV->linesize[2]);
#endif
